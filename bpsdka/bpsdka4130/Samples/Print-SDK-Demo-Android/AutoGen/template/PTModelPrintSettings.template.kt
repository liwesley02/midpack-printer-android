...
{% if ELEMENTS.extend(elementSetDictionary["BRLMPTPrintSettings"].elements) %} {% endif %}
{% macro typeKey(key) -%}
{{ snakeCase(withoutPrefix(inSyntax(key))).upper() }}
{%- endmacro %}
{% set settingName = "ptSettings" %}
{% set settingClass = "PTPrintSettings" %}
...
{% for element in ELEMENTS if not element.key == "BRLMPrinterModel" and not element.key == "BRLMPrintSettingsWorkPath" and not element.key == "BRLMPrintSettingsResolution" %}
    {% if element.type == Type.enum %}
...
{% endfor %}
    ...
    override fun setSettingInfo(key: PrintSettingsItemType, message: Any) {
        ...
        when (key) {
            ...
            PrintSettingsItemType.RESOLUTION -> {
                {{ settingName }}.resolution = PrintImageSettings.Resolution.values().firstOrNull { value -> value.name == message }
            }
{% for element in ELEMENTS if not element.key == "BRLMPrinterModel" and not element.key == "BRLMPrintSettingsWorkPath" and not element.key == "BRLMPrintSettingsResolution" %}
    {% if element.type == Type.enum %}
            PrintSettingsItemType.{{ typeKey(element.key) }} -> {
