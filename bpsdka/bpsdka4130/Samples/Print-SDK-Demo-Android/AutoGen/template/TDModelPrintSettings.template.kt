...
{% if ELEMENTS.extend(elementSetDictionary["BRLMTDPrintSettings"].elements) %} {% endif %}
{% macro typeKey(key) -%}
{{ snakeCase(withoutPrefix(inSyntax(key))).upper() }}
{%- endmacro %}
{% set settingName = "tdSettings" %}
{% set settingClass = "TDPrintSettings" %}

class TDModelPrintSettings(val context: Context, override val printerModel: PrinterModel) : ISimplePrintSettings {
    private var {{ settingName }} = TDPrintSettings(printerModel)
...
{% for element in ELEMENTS if not element.key == "BRLMPrinterModel" and not element.key == "BRLMPrintSettingsWorkPath" and not element.key == "BRLMCustomPaperSize"%}
    ...
    {% elif element.type == Type.string %}
            PrintSettingsItemType.{{ typeKey(element.key) }} -> {
                {{ settingName }}.{{ withoutSurfix(element.exif) }} = message.toString()
            }
    {% elif element.type == Type.object %}
    {% elif element.type == Type.struct %}
    {% elif element.type == Type.uint8 %}
