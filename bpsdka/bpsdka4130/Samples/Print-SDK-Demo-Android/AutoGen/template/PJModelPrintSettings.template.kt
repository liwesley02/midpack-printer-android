...
{% if ELEMENTS.extend(elementSetDictionary["BRLMPJPrintSettings"].elements) %} {% endif %}
{% macro typeKey(key) -%}
{{ snakeCase(withoutPrefix(inSyntax(key))).upper() }}
{%- endmacro %}
{% set settingName = "pjSettings" %}
{% set settingClass = "PJPrintSettings" %}
...
{% for element in ELEMENTS if not element.key == "BRLMPrinterModel" and not element.key == "BRLMPJPrintSettingsPaperSize" %}
...
{% for element in ELEMENTS if not element.key == "BRLMPrinterModel" and not element.key == "BRLMPrintSettingsWorkPath" and not element.key == "BRLMPJPrintSettingsPaperSize"%}
    {% if element.type == Type.enum %}
            PrintSettingsItemType.{{ typeKey(element.key) }} -> {
